...
function searchStudentsForSuggestionCounselor(searchTerm, inputField, statusEl) {
    ...
    let apiUrl;
    if (school) {
        apiUrl = `/guidancemanagment/api/get-students.php?school=${encodeURIComponent(school)}&search=${encodeURIComponent(searchTerm)}&limit=8${gradeParam}`;
        statusEl.textContent = 'Searching school records...';
        statusEl.style.color = '#666';
    } else {
        // search across users_tables when school not provided
        apiUrl = `/guidancemanagment/api/get-students.php?search=${encodeURIComponent(searchTerm)}&limit=8${gradeParam}`;
